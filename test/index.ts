...
import { MultiMetaNft } from "../typechain-types";
import { SignerWithAddress } from "@nomiclabs/hardhat-ethers/signers";
import contractArguments from "../config/contract-arguments";
import { NftContractType } from "../lib/nft-contract-provider";
...
!developmentChains.includes(network.name)
    ...
    : describe(collectionConfig.contractName, function () {
          ...
          let contract: MultiMetaNft;
          //   let deployer: SignerWithAddress;
          //   let fee: string | BigNumber;
          ...
          //       deployer = accounts[0]
          //       await deployments.fixture(["multiMetaNft"])
          //       contract = await ethers.getContract(collectionConfig.contractName)
          //       //   await contract.setMintState("0")
          //       fee = await contract.mintFee()
          //   })
          ...
          it("Check initial data", async function () {
              ...
              expect(await contract.mintFee()).to.equal(getPrice(SaleType.WHITELIST, 1));
          ...
          it("Before any sale", async function () {
              ...
              await expect(
                  contract
                      ...
                      .mintNft(1, { value: getPrice(SaleType.WHITELIST, 1) })
              ).to.be.revertedWith("The contract is paused!");
              await expect(
                  contract.connect(whitelistedUser).whitelistMint(1, [], {
                      value: getPrice(SaleType.WHITELIST, 1),
                  })
              ).to.be.revertedWith("The whitelist sale is not enabled!");
              await expect(
                  contract.connect(holder).mintNft(1, { value: getPrice(SaleType.WHITELIST, 1) })
              ).to.be.revertedWith("The contract is paused!");
              await expect(
                  contract.connect(holder).whitelistMint(1, [], {
                      value: getPrice(SaleType.WHITELIST, 1),
                  })
              ).to.be.revertedWith("The whitelist sale is not enabled!");
              await expect(
                  contract.connect(owner).mintNft(1, { value: getPrice(SaleType.WHITELIST, 1) })
              ).to.be.revertedWith("The contract is paused!");
              await expect(
                  contract.connect(owner).whitelistMint(1, [], {
                      value: getPrice(SaleType.WHITELIST, 1),
                  })
              ).to.be.revertedWith("The whitelist sale is not enabled!");
          ...
          it("Whitelist sale", async function () {
              ...
              // Puase whitelist sale
              await contract.setWhitelistMintEnabled(false);
              await contract.setMintFee(
          ...
          it("Pre-sale (same as public sale)", async function () {
              ...
              await contract.connect(holder).mintNft(2, { value: getPrice(SaleType.PRE_SALE, 2) });
              await contract
                  .connect(whitelistedUser)
                  .mintNft(1, { value: getPrice(SaleType.PRE_SALE, 1) });
              // Sending insufficient funds
              await expect(
                  contract
                      .connect(holder)
                      .mintNft(1, { value: getPrice(SaleType.PRE_SALE, 1).sub(1) })
              ).to.be.rejectedWith(Error, "insufficient funds for intrinsic transaction cost");
              // Sending an invalid mint amount
              await expect(
                  contract
                      .connect(whitelistedUser)
                      .mintNft(await (await contract.maxMintAmountPerTx()).add(1), {
                          value: getPrice(
                              SaleType.PRE_SALE,
                              await (await contract.maxMintAmountPerTx()).add(1).toNumber()
                          ),
          ...
          it("Owner only functions", async function () {
              await expect(
                  contract.connect(externalUser).mintForAddress(1, await externalUser.getAddress())
              ).to.be.revertedWith("Ownable: caller is not the owner");
              await expect(contract.connect(externalUser).setRevealed(false)).to.be.revertedWith(
                  "Ownable: caller is not the owner"
              );
              await expect(
                  contract.connect(externalUser).setMintFee(utils.parseEther("0.0000001"))
              ).to.be.revertedWith("Ownable: caller is not the owner");
              await expect(
          ...
          it("Supply checks (long)", async function () {
              ...
              /** maxMintAmountPerTx not tesing out for all case scenarios. */
              // Tested results passing = 400, 300, 200, 100, 50, 40, 30, 20, 10, 8, 6, 4, 3,
              // Tested results failing = 5, 2, 1
              ...
              await Promise.all(
                  [...Array(iterations).keys()].map(
                      async () =>
                          await contract.connect(whitelistedUser).mintNft(maxMintAmountPerTx, {
              ...
              await expect(
                  contract.connect(holder).mintNft(lastMintAmount + 1, {
                      value: getPrice(SaleType.PUBLIC_SALE, lastMintAmount + 1),
                  })
              ).to.be.revertedWith("Max supply exceeded!");
              await expect(
                  contract.connect(holder).mintNft(lastMintAmount + 2, {
              ...
              await contract.connect(owner).mintNft(lastMintAmount, {
              ...
              await expect(
                  contract
                      .connect(whitelistedUser)
                      .mintNft(1, { value: getPrice(SaleType.PUBLIC_SALE, 1) })
              ).to.be.revertedWith("Max supply exceeded!");
          ...
          //     //   it("Reverts if mint price compliance not met", async function () {
          //     //     await expect(
          //     //         contract.mintNft
          //     //     )
          //     //   })

          //       it("reverts if the mint is closed", async function () {
          //           await contract.setMintState("1")
          //           await expect(
          //               contract.mintNft({
          //                   value: fee,
          //               })
          //           ).to.be.revertedWith("MultiMetaNft__MintNotOpen")
          //       })

          //       it("reverts if the max supply is reached", async function () {
          //           console.log("wait......")
          //           let i: number
          //           for (i = 0; i < 399; i++) {
          ...
          //                   value: fee,
          //               })
          //           ).to.be.revertedWith("MultiMetaNft__MaxSupplyReached")
          //       })

          //       it("fails if payment isn't sent with the request", async function () {
          //           await expect(contract.mintNft()).to.be.revertedWith("MultiMetaNft__NeedMoreETHSent")
          //       })
          ...
          //               })
          //           ).to.be.revertedWith("MultiMetaNft__NeedMoreETHSent")
          //       })
          ...
          //       it("emits an event after Nft is Minted", async function () {
          //           await expect(contract.mintNft({ value: fee })).to.emit(contract, "NftRequested")
          //       })
          //   })
      });
